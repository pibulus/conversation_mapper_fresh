...
export default function CircularNetworkGraph(
  { loading = false }: CircularNetworkGraphProps,
) {
  ...
  function createFullscreenPortal() {
    ...
    modalContainer.className =
      "bg-white rounded-lg shadow-brutal border-4 border-purple-400";
  ...
  function updateVisualization(container: HTMLElement) {
    if (
      !container || !width.value || !height.value || topics.value.length === 0
    ) return;
    ...
    const tooltip = d3.select("body")
      ...
      .attr(
        "class",
        "absolute bg-white text-sm shadow-lg rounded px-2 py-1 border border-gray-300",
      )
    ...
    nodeGroup.selectAll("circle")
      ...
      .on("mouseover", function (event, d) {
        ...
        const connectedIds = relationships.value
          .filter((rel) =>
            rel.source_topic_id === d.id || rel.target_topic_id === d.id
          )
      ...
      .on("mouseout", function () {
        ...
        nodeGroup.selectAll("circle").attr(
          "stroke-width",
          1.5 * sizeMultiplier,
        );
