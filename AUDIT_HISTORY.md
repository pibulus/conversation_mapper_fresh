...
> November 2025 audits of the Conversation Mapper codebase. Key actionable
> findings are captured in `CLAUDE.md` Known Issues. This file preserves the
> historical record.
...
1. **406+ hardcoded `px`/`rem` inline style values** — CSS tokens defined in
   `static/styles.css` `:root` are not used consistently. Theming and responsive
   behavior suffer.
2. **Append flow issues** — Summary is replaced not merged; `statusUpdates` only
   flow forward (completed items can't be re-opened via append).
3. **Naive deduplication** — Action item dedup relies on string matching;
   semantically similar items can slip through.
