...
Deno.test("generateTitle returns trimmed response", async () => {
	...
	const title = await service.generateTitle("Alice: Let's discuss the roadmap.");
...
Deno.test("extractActionItems parses valid JSON array", async () => {
	...
	const items = await service.extractActionItems("Alice will write the report.");
...
Deno.test("extractActionItems converts 'null' string assignee to null", async () => {
	const json = JSON.stringify([{ description: "task", assignee: "null", due_date: "null" }]);
	const service = createGeminiService(mockModel(json));
...
Deno.test("checkActionItemStatus parses valid JSON response", async () => {
	const responseData = [
		{ id: "1", description: "write report", status: "completed", reason: "mentioned done" },
	];
	...
	const updates = await service.checkActionItemStatus("I finished the report.", existing);
...
Deno.test("checkActionItemStatus returns empty array for empty JSON response", async () => {
	...
	const updates = await service.checkActionItemStatus("nothing relevant", existing);
	assertEquals(updates, []);
});
...
Deno.test("generateMarkdown returns trimmed response", async () => {
	...
	const result = await service.generateMarkdown("convert to report", "meeting text");
