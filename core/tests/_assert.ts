...
export function assertStringIncludes(actual: string, expected: string, msg?: string): void {
	if (!actual.includes(expected)) {
		throw new Error(
			msg ??
				`assertStringIncludes failed: "${expected}" not found in "${actual.slice(0, 200)}"`,
		);
	}
}
...
export async function assertRejects(
	...
	try {
		...
		if (e instanceof Error && e.message === "assertRejects: function did not throw") {
			throw e;
		}
		if (errorClass && !(e instanceof errorClass)) {
			throw new Error(
				`assertRejects: expected ${errorClass.name} but got ${(e as Error).constructor?.name}`,
			);
		}
		if (msgIncludes && !(e as Error).message?.includes(msgIncludes)) {
			throw new Error(
				`assertRejects: error message "${(e as Error).message}" does not include "${msgIncludes}"`,
			);
		}
	}
}
