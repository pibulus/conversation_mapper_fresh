...
function createMockAIService(): AIService {
	return {
		...
		async extractActionItems(_input, _speakers, _existing) {
			return [{ description: "Do something", assignee: "Alice", due_date: "2025-12-01" }];
		},
...
Deno.test("processText returns ConversationFlowResult with correct shape", async () => {
	...
	const result = await processText(service, "Alice: we need to ship this.", "conv-1");
...
Deno.test("processAudio returns ConversationFlowResult with correct shape", async () => {
	...
	const result = await processAudio(service, blob, "conv-2");
...
Deno.test("processAudio generates a title from transcription", async () => {
	...
	const result = await processAudio(service, blob, "conv-3");
...
Deno.test("exportConversation delegates to aiService.generateMarkdown", async () => {
	const service = createMockAIService();
	const result = await exportConversation(service, "convert to report", "transcript text");
